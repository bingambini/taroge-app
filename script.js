...
async function init() {
    ...
    try {
        // მნიშვნელოვანია: Google Scripts-ისთვის გვჭირდება redirect: 'follow'
...
function renderHeader(h) {
    ...
    if (h.status !== 'active') { el.style.display = 'none'; return; }
...
function openProductSheet(index) {
    ...
    document.getElementById('sheet-content').innerHTML = `
        <div class="flex flex-col items-center">
            <div class="w-full aspect-square bg-slate-50 rounded-[30px] flex items-center justify-center mb-6"><img src="${p.images.split(',')[0]}" class="w-4/5 object-contain"></div>
...
function goToProductDetails(id) {
    ...
    document.getElementById('details-content').innerHTML = `
        ...
        <div class="mt-6">
            ...
            <button id="dynamic-buy-btn" disabled onclick="handleBuy('${details.id}')" class="w-full bg-slate-200 text-white py-5 rounded-[25px] font-black text-lg transition-all">აირჩიე ზომა და ფერი</button>
        </div>`;
...
function placeOrder() { switchPage('checkout'); document.getElementById('checkout-total-price').innerText = document.getElementById('cart-total-price').innerText; }

function confirmFinalOrder() {
    const name = document.getElementById('order-name').value;
    const phone = document.getElementById('order-phone').value;
    if(!name || !phone) { alert("შეავსეთ სახელი და ტელეფონი"); return; }
    
    // გადახდის მეთოდების რენდერი შენს სტილში
    document.getElementById('payment-options').innerHTML = `
        <div onclick="finishOrder('ნაღდი')" class="bg-white p-5 rounded-[25px] shadow-sm flex items-center gap-4">
            <div class="w-10 h-10 bg-blue-50 text-blue-500 rounded-xl flex items-center justify-center"><i class="fa-solid fa-money-bill"></i></div>
            <span class="font-black text-sm">ნაღდი ანგარიშსწორება</span>
        </div>
        <div onclick="finishOrder('ბარათი')" class="bg-white p-5 rounded-[25px] shadow-sm flex items-center gap-4">
            <div class="w-10 h-10 bg-green-50 text-green-500 rounded-xl flex items-center justify-center"><i class="fa-solid fa-credit-card"></i></div>
            <span class="font-black text-sm">ბარათით გადახდა</span>
        </div>
    `;
...
async function finishOrder(method) {
    ...
    const params = new URLSearchParams({
        ...
        items: items,
        total: total,
        address: document.getElementById('order-address').value + " [მეთოდი: " + method + "]"
    });

    try {
        // აქ ვიყენებთ fetch-ს Google Script-ში ჩასაწერად
        fetch(`${API_URL}?${params.toString()}`, { mode: 'no-cors' });
        
        tg.sendData(JSON.stringify({ action: "order_complete", total: total }));
        localStorage.removeItem('cart');
        alert("მადლობა! შეკვეთა მიღებულია.");
        tg.close();
    } catch (e) {
        alert("შეცდომაა, სცადეთ მოგვიანებით.");
    }
}
